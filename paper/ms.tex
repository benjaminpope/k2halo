...
\noindent The weights are chosen to minimize the modified Total Variation

\begin{equation}
    TV \equiv \frac{\sum_i{|f_i - f_{i-l}|}}{\sum_i {f_i}},
\end{equation}

\noindent which for $l = 1$ is the standard Total Variation and can be seen as the L1 norm on the derivative of $f$ or as a discrete approximation to its arc length. We introduce a lag parameter $l$ in order to allow for flexibility in modelling systematics occurring at different timescales from cadence-to-cadence, and we investigate its effects below. TV is a convex objective function and has analytic derivatives with respect to $w_j$ \citep[calculated with \texttt{autograd};][]{autograd}, and it is therefore extremely fast to optimize and converges well on a global solution.
% smear
...
\begin{figure*}
\plotone{../release/c14/hlsp_halo_k2_llc_200182931_-c14_kepler_v1_lc.png}
\end{figure*}
